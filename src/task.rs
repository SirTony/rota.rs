...
pub type ErrFn = Box<dyn FnMut(Id, Error) + Send + Sync>;
...
impl<F: 'static + Send + Sync + FnMut(Id, CancellationToken) -> Result> Executable for F {
    fn execute(&mut self, id: Id, ct: CancellationToken) -> Result {
        (*self)(id, ct)
    }
}
...
{
    pub fn on_error<F: FnMut(Id, Error) + Send + Sync + 'static>(
        self,
        f: F,
...
impl ScheduledTask {
    ...
    where
        F: FnMut(Id, Error) + Send + Sync + 'static,
    {
